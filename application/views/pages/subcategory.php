...
$(document).ready(function(){
  $(document).on('click','#load_more',function(){
    ...
    var url = "<?= site_url('subcategory/load_more_ajax/'); ?>"+cat
    $('.load_more').hide();
    $('.loding').show();
    $.ajax({
      type:'POST',
      url:"<?= site_url('subcategory/load_more_ajax/'); ?>"+cat,
...
<script>
							    ...
							    $("#min,#max,#sort_by,#lookingfor,#college_id").change(function(){
							        ...
							        var cat = "<?= $this->uri->segment(2) ?>";
							        var url = "<?= site_url('subcategory/minmax/'); ?>"+cat;
							        $(".se-pre-con").fadeIn("slow");
							    	$("#infoi").css(( "z-index" ), 10 );
							    	$.ajax({
								      type:'POST',
								      url:"<?= site_url('subcategory/minmax/'); ?>"+cat,
